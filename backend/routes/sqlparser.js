...
function canonSelectColumns(cols, { ignoreSelectOrder }) {
  const list = (cols || []).map((c) => {
    ...
    if (e.type === "aggr_func") {
      ...
      return `FUNC:${String(e.name).toUpperCase()}(${inner})`;
    }
...
function canonTables(fromArr) {
  ...
  return [...new Set(names.map(s => s?.toLowerCase()))].sort();
}
function canonJoins(fromArr) {
  ...
  for (const f of Array.isArray(fromArr) ? fromArr : [fromArr]) {
    if (!f?.join) continue;
    joins.push({
      join: String(f.join).toUpperCase(),
      table: stripQuote(f.table || "").toLowerCase(),
      on: f.on || null
    });
  }
...
function canonOrderBy(orderby, { ignoreOrderBy }) {
  const list = (orderby || []).map((o) => {
    ...
    const dir = String(o.type || "ASC").toUpperCase();
    return `${key}:${dir}`;
  });
...
export default async function checkSQLStructure(studentSQL, teacherSQL, opts = {}) {
